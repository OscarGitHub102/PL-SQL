...
CREATE OR REPLACE PACKAGE GESTION_CURSO AS
    ...
    PROCEDURE AUMENTAR_PRECIO_CURSO(V_NUM_PROF CURSOS.NUMPRO%TYPE);
    PROCEDURE VISUALIZAR_DATOS_CURSO(V_NUM_CURSO NUMBER);
    PROCEDURE VISUALIZAR_DATOS_CURSO(V_TITULO_CURSO VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY GESTION_CURSO AS
    FUNCTION BUSCAR_CURSO_POR_TITULO(V_NUM_CURSO CURSOS.NUMCUR%TYPE)
    RETURN NUMBER;
...
------------
    ...
    IS
        CURSOR C1 IS 
            SELECT NUMPRO, TITCUR, PRECUR FROM CURSOS WHERE NUMPRO = V_NUM_PROF;
        REG C1%ROWTYPE;
    BEGIN
        OPEN C1;
        FETCH C1 INTO REG;
        WHILE C1%FOUND LOOP
            UPDATE CURSOS SET PRECUR = PRECUR * 1.10 WHERE NUMPRO = V_NUM_PROF;
            FETCH C1 INTO REG;   
        END LOOP;
        IF C1%ROWCOUNT > 0 THEN
            DBMS_OUTPUT.PUT_LINE(chr(10) || 'EL PRECIO DE LOS CURSOS IMPARTIDOS POR EL PROFESOR DE CÓDIGO ' || V_NUM_PROF || ' SE HAN INCREMENTADO CORRECTAMENTE');        
        ELSE
            DBMS_OUTPUT.PUT_LINE(chr(10) || 'NO SE ENCONTRARON CURSOS IMPARTIDOS POR EL PROFESOR DE CÓDIGO ' || V_NUM_PROF);
        END IF;
        CLOSE C1;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                DBMS_OUTPUT.PUT_LINE(chr(10) || 'NO SE ENCONTRÓ EL CURSO CON EL NÚMERO ' || V_NUM_CURSO);    
...
------------
    ...
    BEGIN
        SELECT NUMCUR INTO COD FROM CURSOS WHERE TITCUR = V_NUM_CURSO;
        RETURN COD;
